...
<p>Let us consider the query release problem where a analyst is querying a database with the two queries resulting in database without a given row (D2) and database with the given row(D1) as defined in previous tutorial. We shall again take the small town example in which the sum of 20 people income to $200million.</p>
