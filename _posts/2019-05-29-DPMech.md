...
<p>Suppose we know the queries Q we want to answer in advance for a given dataset D. Instead of answering the queries from the dataseet D , we would want to instead synthesize datasets Ds1 , Ds2 , ..Dsn. These are several synthetic datasets generated from dataset D which can answer Q upto a accuracy of a1,a2,..,an. Each time a query is asked , instead of answering from D , we choose a synthetic dataset from the synthesized datasets using exponential mechanism and answer the queries. Suppose few datasets have closeby accuracies , the datasets with higher accuracy have a larger chance of being chosen. But , that depends on how large the accuracy difference is. If the accuracy difference isnt large , either of the datasets with similar accuracies can be choosen. 
