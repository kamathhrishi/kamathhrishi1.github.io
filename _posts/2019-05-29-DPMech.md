...
<p>To solve the above bidding problem , we can have a range of continious values from 0.5 to 3.0 with a step of 0.1. The utility score will be the revenue generated if the item is sold for a given price. We calculate for utility for all the values and find the maximum sensitivity denoted by ∆u. The price of item p will be set by calculating exp(εu(x, r)/∆u) normalized by sum of all exp(εu(x, r)/∆u) for all the continious values in the range. The values generated by the mechanism is the probability with which each of the values would be selected. </p>
