...
The previous tutorial defined DP as two probability distributions being close to each other by a factor of exp(epsilon). We put this definition into practice by adding carefully noise to every query from a probability distribution that is calibrated according to the definition of DP. 
